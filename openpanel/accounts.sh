...
function _create_accounts() {
  ...
  if [[ -f ${DATABASE_ACCOUNTS} ]]; then
    ...
    local LOGIN PASS UID_OVERRIDE USER_ATTRIBUTES
    while IFS=$'|' read -r LOGIN PASS UID_OVERRIDE USER_ATTRIBUTES; do
      ...
      local EFFECTIVE_UID="${UID_OVERRIDE:-${DMS_VMAIL_UID}}"
      local EFFECTIVE_GID="${UID_OVERRIDE:-${DMS_VMAIL_GID}}"

      DOVECOT_USERDB_LINE="${LOGIN}:${PASS}:${EFFECTIVE_UID}:${EFFECTIVE_GID}::/var/mail/${DOMAIN}/${USER}/home::${USER_ATTRIBUTES}"
